...
iter = 50 # iterations of NPMLE_FW
...
np.random.seed(626)
X,y,C = generate_test_data(n,iter, b1, b2, b3,pi1,pi2,sigma,func)
#-----------------------------------------------------------#
...
for i in range(len(x_list)):
    ...
    if func.__name__ == 'lin_func':
        plt.plot(y, sum(alpha4[i]*scipy.stats.norm.pdf( y-func(B4[:,i],[1,x]), 0, sigma_array4[i]) \
               for i in range(len(alpha4))),color = 'tab:red',label = 'EM',linestyle = line_styles[3])
    
    plt.plot(y,pi1*scipy.stats.norm.pdf(y - func(b1,[1,x]), 0, sigma)+pi2*scipy.stats.norm.pdf(y-func(b2,[1,x]),0, sigma)+\
    (1-pi1-pi2)*scipy.stats.norm.pdf(y-func(b3,[1,x]),0, sigma),color = 'tab:blue',label = 'Truth',linestyle =line_styles[0])

    plt.plot(y, sum(alpha2[i]*scipy.stats.norm.pdf( y-func(B2[:,i],[1,x]), 0, sigma_cv) \
    for i in range(len(alpha2))),color = 'tab:green',label = 'NPMLE',linestyle = line_styles[1])
        
    plt.plot(y, sum(alpha1[i]*scipy.stats.norm.pdf( y-func(B1[:,i],[1,x]), 0, sigma) \
